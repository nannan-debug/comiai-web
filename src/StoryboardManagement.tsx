      ...
      <div className="flex-1 flex overflow-hidden gap-3">
        ...
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <span className="text-sm font-bold text-slate-700 flex items-center gap-2">
            <BookOpen className="w-4 h-4 text-violet-500" /> 剧本上下文
          </span>
        ...
        <div className="flex-1 overflow-y-auto p-4 text-xs text-slate-600 leading-relaxed space-y-4 bg-slate-50/30">
          {scriptLines.map((line, idx) => (
            <div key={idx} className={idx === 2 ? 'border-violet-400 border-l-2 pl-3 bg-violet-50/50 -ml-4 p-2 rounded-r shadow-sm' : 'pl-3'}>
              {line.includes('老兵甲') ? (
                <span>
                  {line.split('老兵甲')[0]}
                  <span className="text-violet-600 font-bold bg-violet-100 px-1 rounded mx-0.5 shadow-sm">老兵甲</span>
                  {line.split('老兵甲')[1]}
                </span>
              ) : (
                line
      ...
      <main className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden relative min-w-0">
        ...
        <div className="border-b border-slate-100 px-6 py-3 shrink-0 flex items-center justify-between z-30 bg-slate-50/50">
          <div className="flex items-center gap-4">
            <button 
              className="text-slate-400 hover:text-violet-600 p-1.5 rounded-lg hover:bg-violet-50 transition-colors border border-transparent hover:border-violet-100" 
                    ...
                    <div className="absolute top-full left-0 mt-1 w-44 bg-white border border-slate-200 rounded-lg shadow-xl py-1 z-50 max-h-64 overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2 duration-200">
                      {styles.map(style => (
                        <button 
                          ...
                          className={`w-full text-left px-3 py-1.5 text-[11px] transition-colors flex items-center gap-2.5 ${selectedStyle === style.id ? 'bg-violet-50 text-violet-600 font-bold' : 'text-slate-700 hover:bg-slate-50'}`}
                        >
                  ...
                  {isOrientationDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-xl py-1 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                      <button 
                        ...
                        className={`w-full text-left px-3 py-1.5 text-[11px] transition-colors flex items-center gap-2.5 ${selectedOrientation === '16:9' ? 'bg-violet-50 text-violet-600 font-bold' : 'text-slate-700 hover:bg-slate-50'}`}
                      >
                        <div className={`w-3 h-1.5 border rounded-[1px] ${selectedOrientation === '16:9' ? 'border-violet-500' : 'border-slate-400'}`}></div>
                        <span>16:9 (横屏)</span>
                      </button>
                      <button 
                        onClick={() => { setSelectedOrientation('9:16'); setIsOrientationDropdownOpen(false); }}
                        className={`w-full text-left px-3 py-1.5 text-[11px] transition-colors flex items-center gap-2.5 ${selectedOrientation === '9:16' ? 'bg-violet-50 text-violet-600 font-bold' : 'text-slate-700 hover:bg-slate-50'}`}
                      >
                        <div className={`w-1.5 h-3 border rounded-[1px] ${selectedOrientation === '9:16' ? 'border-violet-500' : 'border-slate-400'}`}></div>
                        <span>9:16 (竖屏)</span>
                      </button>
                    </div>
                  )}
            ...
            <div className="flex bg-slate-200/50 p-0.5 rounded-xl border border-slate-100 ml-2 shadow-inner">
              <button 
                ...
                className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 ${
                  globalMode === 'image-video' ? 'bg-white shadow-sm text-violet-600' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              <button 
                ...
                className="px-3 py-1.5 text-[11px] font-bold bg-white text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-50 hover:text-violet-600 flex items-center gap-1.5 shadow-sm transition-colors"
              >
              ...
              {showModelMenu && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-4 z-50">
                  <div className="text-xs font-bold text-slate-800 mb-3 pb-2 border-b border-slate-100 flex items-center gap-2">
                    <Settings2 className="w-3.5 h-3.5 text-violet-500" /> 批量生成默认引擎
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 mb-1.5 block uppercase tracking-wider">默认生图模型</label>
                      <select className="w-full text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200 rounded-xl p-2 outline-none hover:border-violet-300 cursor-pointer focus:ring-2 focus:ring-violet-500/20 transition-all shadow-inner">
            ...
            <button className="px-4 py-1.5 text-[11px] font-bold bg-slate-800 text-white rounded-xl hover:bg-slate-900 flex items-center gap-1.5 shadow-md active:scale-95 transition-all">
              <Play className="w-3.5 h-3.5 fill-current text-violet-400" /> 批量执行生成
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 pb-20 custom-scrollbar">
          ...
          <div className="pt-2 pb-6">
            <button 
              ...
              className="w-full border-2 border-dashed border-slate-300 rounded-2xl flex items-center justify-center py-6 text-slate-500 hover:text-violet-600 hover:border-violet-400 hover:bg-violet-50 cursor-pointer transition-all shadow-sm bg-white/50 active:scale-[0.99]"
            >
...
function ShotCard({ shotNumber, onDelete, onNext }: { key?: React.Key, shotNumber: number, onDelete: () => void, onNext: () => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex transition-colors hover:border-violet-300 group/card">
      {/* 左侧序号栏 */}
      <div className="bg-slate-50/50 w-12 border-r border-slate-100 flex flex-col items-center pt-5 shrink-0 gap-3 group relative">
        ...
        <div className="flex flex-col gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity">
          <button onClick={onNext} className="text-violet-500 hover:text-violet-700 bg-violet-50 shadow-sm border border-violet-200 rounded-lg p-1.5 transition-colors" title="进入精修模式">
      ...
      <div className="flex-1 p-4 grid grid-cols-12 gap-5">
        ...
        <div className="col-span-3 border-r border-slate-100 pr-5 flex flex-col gap-4">
          ...
          <div className="pt-2 border-t border-slate-50">
            ...
            <div className="flex gap-2 flex-wrap">
              <div className="w-10 h-10 rounded-xl border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 hover:text-violet-500 hover:border-violet-400 hover:bg-violet-50 cursor-pointer transition-colors bg-white shadow-sm">
        ...
        <div className="col-span-5 border-r border-slate-100 pr-5 flex flex-col">
          <label className="text-[10px] text-slate-400 uppercase font-bold mb-2 flex items-center justify-between tracking-wider">
            画面内容与AIGC提示词
            <span className="text-[9px] font-bold text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded cursor-pointer hover:bg-violet-100 transition-colors shadow-sm border border-violet-100">AI 优化</span>
          </label>
          <textarea 
            className="w-full text-xs font-medium text-slate-700 bg-slate-50 border border-slate-200 rounded-xl p-2.5 h-20 resize-none outline-none focus:border-violet-500 focus:bg-white focus:ring-2 focus:ring-violet-500/20 transition-all leading-relaxed mb-3 shadow-inner"
          ...
          <div className="mt-auto grid grid-cols-2 gap-3 text-xs bg-white rounded-xl">
            <div>
              ...
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 outline-none focus:border-violet-500 focus:bg-white text-slate-700 placeholder-slate-400 font-medium shadow-inner transition-colors" placeholder="暂无台词" />
            </div>
            <div>
              <span className="text-[9px] text-slate-400 font-bold block mb-1">音效</span>
              <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 outline-none focus:border-violet-500 focus:bg-white text-slate-700 font-medium shadow-inner transition-colors" defaultValue="柴火爆裂声，风声沉闷" />
            </div>
          </div>
        </div>
        ...
        <div className="col-span-4 flex flex-col relative">
            ...
            <div className="flex bg-slate-200/50 p-0.5 rounded-lg border border-slate-100 shadow-inner">
              <button 
                ...
                className={`px-2 py-0.5 rounded-md text-[10px] font-bold transition-all flex items-center gap-1 ${
                  cardMode === 'image' ? 'bg-white shadow-sm text-violet-600' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
          ...
          {cardMode === 'image' ? (
            <div className="w-full h-full flex gap-2 min-h-[90px] animate-in fade-in duration-300">
              <div className="flex-1 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center bg-slate-50 cursor-pointer hover:border-violet-400 hover:bg-violet-50 text-slate-400 transition-colors group shadow-sm">
                <ImageIcon className="w-5 h-5 mb-1 group-hover:text-violet-500 transition-colors" />
                <span className="text-[10px] font-bold text-slate-600 group-hover:text-violet-600">生成关键帧</span>
              </div>
